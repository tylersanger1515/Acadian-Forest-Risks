...
with right:
    ...
    q = st.text_input(
        ...
        placeholder=(
            ...
            "top 4 largest in NB • counts by province • total hectares by province • "
            "where is fire 68622 • how far is fire 68622 from Halifax • "
            "started last 7 days • older than 3 days • still 0.1 ha after 5 days"
        ),
        key="q_fires",
    )
    ...
    # number words → ints for day parsing
    _NUM_WORDS = {
    ...
    # size phrases (bigger/larger/over/under/below…)
    def parse_size_range(text):
        ...
        if any(w in t for w in ["over","more than",">=","at least","minimum","min ","greater than","bigger than","larger than",">"]):
            return ("min", nums[0]) if nums else None
        if any(w in t for w in ["under","less than","<=","at most","maximum","below","max ","<","smaller than","lesser than"]):
            return ("max", nums[0]) if nums else None
        return None

    # time window: last/past/older/ago (+ number words)
    def parse_days_window(text: str):
        ...
        m = re.search(r'(\d+)\s+day(?:s)?\s+ago', s2)  # “started 7 days ago”
        if m: d = int(m.group(1));  return (None, end - dt.timedelta(days=d), "older")
    ...
    # GEO intent parsing (near/within/closest…)
    def parse_geo(text):
        t = (text or "").lower().strip()
        # within 40 km of Halifax / 40km near Halifax / 40 km from Halifax
    ...
    # geocode using your helper (already defined above in your app)
    ...
    def render_qna(q_text: str):
        try:
            ...
            # control synonyms (OC / UC / BH + hyphens)
            want_ctrl = None
            tnorm = text.replace("-", " ")
            if re.search(r"\bout of control\b|\booc\b|out of-control|outofcontrol", tnorm):
                want_ctrl = "out of control"
            elif re.search(r"\bbeing held\b|\bbh\b|\bheld\b|being held", tnorm):
                want_ctrl = "being held"
            elif re.search(r"\bunder control\b|\buc\b|\bcontained\b|\bcontrolled\b|under control", tnorm):
            ...
            # top N largest (uses current subset)
            ...
            # largest in current subset (e.g., “largest in NB”)
            ...
            # where is fire 68622 / location of <id>
            ...
            if any(k in text for k in ["where is","what location","location of","loc of","coords of"]) and m_id:
                ...
                if not f:
                    ...
                    st.write(
                        f"- {_prov(f)} · {_sizeha(f):,.1f} ha · {f.get('control','—')} · "
                        f"coords: {lat:.4f}, {lon:.4f} · Started {_started_s(f) or '—'}"
                    )
                return

            # NEW: how far is fire <id> from <place>
            m = re.search(r'how\s+far\s+(?:is\s+)?(?:fire\s+)?(\d{3,6})\s+(?:from|to)\s+(.+)', text)
            if m:
                fid, place = m.group(1), m.group(2).strip()
                f = next((x for x in fires if str(x.get("name")) == fid), None)
                if not f:
                    st.info(f"Fire {fid} not found."); return
                g = geocode_place(place)
                if not g:
                    st.info(f"Couldn’t locate “{place}”. Try including province or postal code."); return
                lat0, lon0, lbl = g
                d = haversine_km(lat0, lon0, _lat(f), _lon(f))
                st.markdown(f"**Distance — fire {fid} to {lbl}: {d:.1f} km**")
                st.write(fmt_fire_line(f))
                return

            # still 0.1 ha after N days
            if "still 0.1" in text or ("0.1 ha" in text and ("after" in text or "older than" in text or "later" in text)):
            ...
            if mode == "within" and place:
                ...
                if not show_km:
                    st.info(f"Couldn’t locate “{place}”. Try including the province or a postal code (e.g., 'Halifax NS' or 'B3H 1X1').")
                    return
            ...
            # if a time phrase was used, show the filtered list
            if (d_from or d_to) and any(w in text for w in ["start","started","last","past","older","ago","week","day","days"]):
                if not subset:
